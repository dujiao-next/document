...
`dujiao-next` 二進位內建 `admin` 子指令,用於伺服器端維運操作:列出管理員、重設 2FA、重設密碼。常見使用情境是**超級管理員忘記密碼或遺失 2FA 裝置時的緊急還原**。
...
> 與 `dujiao-next`(server 啟動)共用同一份設定。不會啟動 HTTP / worker / 內嵌前端,不列印 banner,僅初始化資料庫後執行子指令並退出。
...
```bash
dujiao-next admin list-admins                            # 列出所有管理員
dujiao-next admin reset-2fa --username <name>            # 重設指定管理員的 2FA
dujiao-next admin reset-password --username <name>       # 互動式重設密碼(兩次確認)
dujiao-next admin reset-password --username <name> --password <new>  # 一次性提供新密碼
```
...
```bash
cd /opt/dujiao          # 與 server 同一工作目錄(可讀取 config.yml)
./dujiao-next admin list-admins
./dujiao-next admin reset-password --username admin
```
...
```bash
# 假設 WorkingDirectory=/opt/dujiao,User=dujiao
...
- **shell 權限即超級管理員權限**:任何能執行 `dujiao-next admin ...` 的帳號都能改超級管理員密碼或清 2FA,務必限制伺服器 shell 存取。
