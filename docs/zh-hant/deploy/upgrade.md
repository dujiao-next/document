...
| Docker 鏡像 | `dujiaonext/api`、`dujiaonext/user`、`dujiaonext/admin` | 只有 `dujiaonext/dujiao-next`（舊名 `dujiaonext/api` 仍作為過渡別名同步推送） |
| 容器數量 | 4-5 個 | 2-3 個 |
...
3. 修改 compose 檔案：**刪除 `user` 與 `admin` 兩個 service**，只保留 `redis`（+ `postgres`）與 `dujiao-next`。
...
升級前務必閱讀目標版本的 [GitHub Release 說明](https://github.com/dujiao-next/dujiao-next/releases)，了解：
...
升級包含支付回調金額/幣種嚴格校驗或 DujiaoPay 配置變更時：

- 在後台篩選 `provider_type=dujiaopay` 且狀態為「待支付」的記錄，優先等待其支付完成或過期後再升級。
- 如果必須帶著在途訂單升級，確認對應渠道的 `fiat_currency` 與這些支付記錄的幣種一致；在途訂單清空前不要修改該渠道的法幣配置。
- 記錄目前六個預置角色（`readonly_auditor`、`operations`、`support`、`integration`、`finance`、`system_admin`）的測試管理員，供升級後做權限冒煙。

新版會在簽名、渠道、第三方訂單號、商戶訂單號和金額全部匹配，且舊支付記錄沒有新版法幣快照時，受限採納 Webhook 中的簽名幣種；這項升級前檢查仍可避免依賴相容分支。已有新版快照的支付若發生幣種不一致，成功回調仍會被安全校驗拒絕。若已經發生，應保留原始回調與支付記錄並進行人工對賬，不要手工繞過金額或幣種校驗。
...
6. **支付回調**：確認支付回調正常運作
7. **郵件通知**：確認郵件發送功能正常
8. **預置角色權限**：分別使用六個預置角色的測試管理員登入，確認選單可見性、唯讀/寫入權限及 403 邊界符合預期
