...
The `dujiao-next` binary ships with a built-in `admin` subcommand for server-side operations: list administrators, reset 2FA, reset passwords. The most common use case is **emergency recovery when a super admin forgets their password or loses their 2FA device**.
...
> The subcommand shares the same configuration as `dujiao-next` (server startup). It does not start HTTP / worker / embedded frontend, does not print the banner, and exits immediately after the subcommand finishes. Only the database is initialized.
...
```bash
dujiao-next admin list-admins                            # List all admins
dujiao-next admin reset-2fa --username <name>            # Reset 2FA for a specific admin
dujiao-next admin reset-password --username <name>       # Interactive password reset (two-time confirmation)
dujiao-next admin reset-password --username <name> --password <new>  # Provide new password inline
```
...
```bash
dujiao-next admin reset-2fa --username admin
```
...
```bash
dujiao-next admin reset-password --username admin
新密码:
再次输入:
...
```bash
dujiao-next admin reset-password --username admin --password 'StrongPwd!2026'
```
...
```bash
echo 'StrongPwd!2026' | dujiao-next admin reset-password --username admin
```
...
```bash
cd /opt/dujiao          # Same working directory as the server (so config.yml is readable)
./dujiao-next admin list-admins
./dujiao-next admin reset-password --username admin
```
...
```bash
# Assuming WorkingDirectory=/opt/dujiao, User=dujiao
sudo -u dujiao /opt/dujiao/dujiao-next admin reset-password --username admin
```
...
```bash
sudo -u www /www/wwwroot/dujiao-next/dujiao-next admin reset-password --username admin
```
...
- **Shell access ≈ super admin access**: anyone who can run `dujiao-next admin ...` can change the super admin password or clear 2FA. Tightly control server shell access.
